...
async def _scan_filing(
    ...
    """Download one filing's primary document and extract announcements."""
...
def _dedupe_authorizations(
    ...
    single filing it can be picked up by several overlapping buyback phrases
    (e.g. both "repurchase program" and "authority to repurchase"). We key on
    (amount, announcement-date) -- deliberately *not* on the matched phrase --
    so those variants collapse, keeping the earliest filing (the original
    announcement) for each distinct authorization.
    """
    ...
    for ann in sorted(announcements, key=lambda a: a.filing_date):
        ...
        key = (amount_key, ann.announcement_date)
        seen.setdefault(key, ann)
...
def _dedupe_references(
    ...
    overlapping phrases; surfacing one reference per filing is sufficient.
    """

    seen: dict[str, BuybackAnnouncement] = {}
    for ann in sorted(announcements, key=lambda a: a.filing_date):
        seen.setdefault(ann.filing_url, ann)
    return list(seen.values())


@app.get("/api/buybacks/{ticker}", response_model=BuybackResponse)
async def get_buybacks(
    ticker: str, include_references: bool = False
) -> BuybackResponse:
    ...
    also return reference/execution mentions.
    """
    ...
    async with EdgarClient() as client:
        ...
        try:
            filings = await fetch_recent_filings(client, cik)
        except EdgarError as exc:
